...
abstract class NewRoom {
  aRoom: ARoom;
  constructor() {
    this.aRoom = {};
  }
  add(room: Room) {
    this.aRoom[uuidv4()] = [room.nickname, room.id];
  }
}
...
class Dm extends NewRoom {
  addBox(): ARoom {
    dbService
      .doc(`Users/${authService.currentUser.uid}`)
      .onSnapshot((documenta) => {
        const dmList = documenta.data().dm;
        dmList.forEach((inn: string) => {
          dbService
            .doc(`Users/${inn}`)
            .get()
            .then((friend) => {
              this.add(new Room(friend.data().nickname, inn));
            });
        });
      });
    return this.aRoom;
  ...
  addBox2(aroom: ARoom) {
      console.log(aroom)
      console.log(aroom.key)
    Object.keys(aroom).forEach((inn:string) => {
    console.log(inn)
      ...
      button.addEventListener("click", () => {
        ...
        dbService.doc(`Users/${inn}`).update({
          dm: firebaseInstance.firestore.FieldValue.arrayRemove(
        ...
        dmList.forEach((inn) => {
          ...
          button.addEventListener("click", () => {
            ...
            dbService.doc(`Servers/${inn}`).update({
              members: firebaseInstance.firestore.FieldValue.arrayRemove(
                authService.currentUser.uid
              ),
            });
            dbService
                      ...
                      .then((doc) => {
                        if (doc.exists) {
                          if (newFriend != authService.currentUser.uid) {
                              ...
                              .then((doca) => {
                                ...
                                if (!dmList.includes(newFriend)) {
                                  dbService
                                    .doc(`Users/${authService.currentUser.uid}`)
                                    .update({
                                      dm: firebaseInstance.firestore.FieldValue.arrayUnion(
                                        newFriend
                                      ),
                                    });
                            ...
                            alert(
                              "You can't friend yourself. Are you that lonely?"
                            );
                          }
                        } else {
        ...
        <div className={i.servers}>
          ...
          <div className={i.fcServer}>
            ...
            <button
              onClick={() => {
                ...
                dbService.doc(`Users/${authService.currentUser.uid}`).update({
                  server:
                    firebaseInstance.firestore.FieldValue.arrayUnion(serverID),
                });
              }}
            >
              Create
          ...
          <div className={i.fcServer}>
            ...
            <button
              onClick={() => {
                dbService
                  .doc(`Servers/${joinServer}`)
                  .get()
                  .then((doc) => {
                    if (doc.exists) {
                      dbService.doc(`Servers/${joinServer}`).update({
                        members:
                          firebaseInstance.firestore.FieldValue.arrayUnion(
                            authService.currentUser.uid
                          ),
                      });
                      dbService
